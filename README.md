...
- `MINECRAFT_WEBHOOK_SECRET`과 plugin `webhook.shared-secret`은 같은 값이어야 합니다.
- 후원 효과 대상은 config가 아니라 게임 안에서 `/chzzk target set <player>`로 지정합니다.
...
Socket.IO와 CHZZK Session 세부 계약은 `docs/bridge/chzzk-auth-and-session.md`를 따릅니다. Webhook payload, HMAC, 중복 한계는 `docs/bridge/webhook-protocol.md`가 canonical입니다.
...
| 증상                                  | 먼저 확인할 곳                                                                               |
| ------------------------------------- | -------------------------------------------------------------------------------------------- |
| compose config 단계에서 실패          | `.env`의 `EULA`, `CHZZK_CLIENT_ID`, `CHZZK_CLIENT_SECRET`, `CHZZK_CHANNEL_ID`, `MINECRAFT_WEBHOOK_SECRET` |
