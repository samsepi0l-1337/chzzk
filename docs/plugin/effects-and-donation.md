...
| `30000`  | `TNT`               | target 반경 3블록 안에 TNT 3개(90%)·4개(9%)·5개(1%) 즉시 소환, 2초 뒤 폭발  |
...
금액 tier는 `config.yml`이 아니라 `DonationTier` enum으로만 정의한다. 현재 `config.yml`에는 효과별 runtime 설정이 없다.
...
- `RANDOM_TELEPORT`는 후원 수신 시점의 target 위치 기준 X와 Z를 각각 `-1000..1000` 범위에서 새로 무작위 선택한다. 플러그인은 target이 online이면 enable 20틱 뒤부터 10틱마다 target 주변 청크를 최대 8개씩 Paper urgent async chunk load로 미리 요청한다. 효과 실행 시에는 새 랜덤 좌표 후보의 청크를 동기 로드하고 `getHighestBlockYAt` 기준 지상 위치를 최대 24회 검사한다. 발·머리 칸은 통과 가능해야 하고 물/용암이면 안 되며, 발 아래는 solid이고 액체가 아니어야 한다. 물속·용암 속·땅속·공중 부유 위치는 거부한다. 안전한 후보를 찾지 못하면 현재 위치에 fallback한다. target은 목적지 주변 11x11 청크를 urgent async load로 요청한 뒤 즉시 텔레포트한다. 텔레포트 시 enderman teleport 사운드를 재생한다.
- 랜덤 선택은 `RandomPools` 값에서 `Random`으로 선택한다.
- `COMBAT_MOB`와 `THREE_COMBAT_MOBS`는 각 소환마다 0.5% 확률로 `WITHER`를 뽑고, 실패하면 `RandomPools.combatMobs()`에서 무작위로 선택한다.
- `TNT`는 target 기준 반경 3블록 안에 TNT 3개(90%)·4개(9%)·5개(1%)를 즉시 소환하고 fuse를 `40 ticks`로 설정해 2초 뒤 폭발시킨다. 소환 시 target 위치에 primed TNT 사운드를 재생한다. 최초 TNT 폭발을 기다리는 후속 소환 체인은 없다.
