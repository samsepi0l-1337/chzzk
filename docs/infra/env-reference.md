...
`CHZZK_CHANNEL_ID`는 효과를 실행할 스트리머의 채널 식별자다. CHZZK Session 구독은 OAuth 토큰 기준으로 열리며, bridge는 수신된 `DONATION.channelId`가 이 값과 정확히 일치할 때만 plugin webhook을 호출한다.
...
| `CHZZK_CHANNEL_ID`      | `target-streamer-channel-id`             | Minecraft 효과를 실행할 CHZZK 이벤트 채널 ID                           |
